...
class RiskAdjustmentScorecardsScreen extends StatefulWidget {
  const RiskAdjustmentScorecardsScreen({super.key});
...
class _RiskAdjustmentScorecardsScreenState
  ...
  RiskAdjustmentPractice get _activePractice => riskAdjustmentPractices.first;
  ...
  Widget _buildPracticeBanner() {
    return Row(
      children: [
        Expanded(
          child: _buildPracticeCard('Practice Name', _activePractice.name),
        ),
        const SizedBox(width: 16),
        Expanded(
          child: _buildPracticeCard('TIN#', _activePractice.tin),
        ),
      ],
    );
  }
  ...
  Widget _buildScorecardToolbar() {
    return Row(
      ...
      children: [
        SizedBox(width: 280, child: _buildMcoFilter()),
        const Spacer(),
        OutlinedButton(
          onPressed: () {
            final mco =
  ...
  TableRow _buildScorecardDataRow(ScorecardRow row) {
    ...
    return TableRow(
      decoration: row.alert
          ? BoxDecoration(color: _alertRed.withValues(alpha: 0.04))
          : null,
      children: [
